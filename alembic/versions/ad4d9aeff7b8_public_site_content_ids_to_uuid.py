"""public_site content ids to uuid (about_tabs, programm_hints, quotes)

Revision ID: ad4d9aeff7b8
Revises: 61330e9e0ca8
Create Date: 2026-09-01 11:39:25.716284

Three structurally identical leaf tables (no incoming foreign keys),
bundled into one migration - same Phase A + Phase C in one step, and same
CONCURRENTLY-index-straight-to-primary-key cutover, as the pattern-setting
61330e9e0ca8_public_site_social_links_id_to_uuid.py. See that migration's
docstring for the full rationale; this one only carries the per-table
mechanics.
"""
...
revision: str = "ad4d9aeff7b8"
down_revision: str | Sequence[str] | None = "61330e9e0ca8"
...
def downgrade() -> None:
    ...
    Not loss-free - same caveat as 61330e9e0ca8 and
    9f8ff3c10cb2_scheduled_task_runs_id_to_autoincrement.py: a freshly
    created integer sequence has no relationship to any UUID that may
    already have circulated. Emergency rollback shortly after deploy
